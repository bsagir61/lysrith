...
func _rebuild() -> void:
	...
	var tag_text: String = "Classification: " + (String(r["hidden_tag"]) if r["tag_revealed"] else "UNKNOWN")
	name_box.add_child(UITheme.label(tag_text, UITheme.FS_SMALL, UITheme.TEXT_DIM))
	header.add_child(name_box)
	var intel_badge := UITheme.label("Intel Level %d/3" % intel_lv, UITheme.FS_SMALL, UITheme.ACCENT_DIM)
	...
	if intel_lv == 0:
		_content.add_child(UITheme.label("Run Map Signals here to reveal more.", UITheme.FS_TINY, UITheme.TEXT_DIM))

	var plan := UITheme.button("CHOOSE AGENT", "primary", true)
...
func _stat_row(label_text: String, value: float, color: Color, known: bool) -> Control:
	...
	if known:
		...
		var unknown := UITheme.label("Need Intel", UITheme.FS_SMALL, UITheme.EDGE_BRIGHT)
